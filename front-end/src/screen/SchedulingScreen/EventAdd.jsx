...
const EventAdd = () => {
  ...
  const verifyValid = () => {
    if (eventPaylad.name === "" || eventPaylad.name === null) {
      ...
      createEvent(eventPaylad);
      dispatch(changeAddOpen());
    }
  }

  return (
    <Dialog open={event.addOpen} maxWidth="md" fullWidth={true}>
      <DialogTitle>
        Adicionar evento
        <IconButton variant="close" onClick={() => dispatch(changeAddOpen())}>
          <Close />
        </IconButton>
      </DialogTitle>
      <DialogContent sx={{ height: "40dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-around" }}>
        ...
        <Typography position="center">
          Informe os dados para adicionar um evento.
        </Typography>
        <Box width={"80%"}>
